...
- **`easy loraNames`** (display name "Lora Names") — upstream class `setLoraName`, from
  `py/nodes/util.py`. Logic is unmodified from upstream; the one helper it relied on from a
  sibling module (`libs.utils.AlwaysEqualProxy`, a "compares equal to any type" string subclass)
  is inlined directly in `nodes.py` instead of vendoring Easy-Use's own module tree.
