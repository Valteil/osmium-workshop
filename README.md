...
3. **Comfy Bridge** (`comfy-bridge/`) — a simpler front end for the ComfyUI backend, built around
   ...
   TCP 8188 in your firewall. Each generation can produce up to three results (Pass 1, Pass 2,
   Upscaled, each its own saved file), shown as swipeable preview slides with thumbnails —
   desktop's preview panel also drag-resizes horizontally. A ComfyUI Terminal panel (a
   collapsible section on desktop, its own slideout on mobile) streams the real server
   stdout/stderr live during generation, straight from ComfyUI's own internal logs API.
