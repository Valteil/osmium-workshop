...
1. Grab a build (see [Development](#development) if you're building from source instead of a
   release zip).
2. Run `Dataset Manager Studio.exe` (or the equivalent for your OS). It's portable — no installer.
...
1. **Dataset Manager Studio (desktop)** — this app: the full dataset tag editor described below.
   Portable Electron app — unzip, run, done.
2. **Dataset Manager Studio (mobile)** — an Android app for tagging training datasets on the go:
...
npm run refresh-app   # build, then regenerate the portable test build at THIS directory's root
                        # (Dataset Manager Studio.exe, resources/, data/, etc. — see Project structure)
```
...
```bash
ELECTRON_ENABLE_LOGGING=1 "./Dataset Manager Studio.exe" --enable-logging=stderr
```
...
<project root>/
  ...
  Dataset Manager Studio.exe, resources/, locales/, data/, chrome_*.pak, *.dll, ...
...
project-memory graph (`mem:core` and onward, project name `dataset-manager-studio-electron`, rooted
