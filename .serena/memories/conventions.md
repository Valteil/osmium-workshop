...
**Single mode: static preview + click-to-lightbox (2026-09-24).** Normal Single view no longer has an inline zoomable image. `view.ts`'s `buildSinglePreview(e)` renders a small static preview (`.single-preview`); clicking it opens `shared-ui.ts`'s `showImageLightbox(e.objectUrl)` (fullscreen, wheel/pinch zoom, drag-pan, Esc/click-outside to close) — the same lightbox sequential mode and the card modal already use. The freed space widens the tag panel (`.single-panel-main` = flex:1) so it scrolls far less. The old zoom slider/`applySingleTransform` were removed; `buildSingleImgSide` (zoom/pan) is now SEQUENTIAL-ONLY. The toolbar counter `#singlePos` is built by `renderSinglePos()` as an editable field: type a number + Enter/blur to jump (`singleIndex` clamp + re-render).
