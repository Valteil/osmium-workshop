...
**Aspect-ratio bucketing ("Bucket Images") — the renderer owns the dataset's file I/O; the main process only computes.** The dataset folder is a `FileSystemDirectoryHandle` held by the renderer, so `src/bucket-local.ts` (main) has NO path to it — `bucketImage()` takes image BYTES and returns PNG BYTES, and `src/renderer/bucket-images.ts` does every move/copy/write. Exactly the WD14 split; don't try to pass a folder path across IPC. `src/bucket-core.ts` holds the pure bucket math (`getValidBuckets`/`getBestBucket`/`isBucketSize`), imported by both sides. **Run u2net on CPU** — DirectML burns CPU forever building its session and freezes the whole main process, with no error (`notes/Pitfalls/DirectML-Hangs-On-U2Net-Graph.md`); 320x320 CPU inference is ~1.5s including the 176MB model load. Bucketing originals land in `original_images/` as entries that are `disabled: true` AND `original: true` — disabled so every existing mass/auto tool skips them, `original` so they get their own view instead of the Disabled one. Any new `disabled`-keyed loop that should also ignore originals must check `.original` too. Full note: `notes/Features/Bucket-Images.md`.
