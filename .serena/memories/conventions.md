...
**Theme Studio (Custom theme):** `src/renderer/theme-spec.ts` is the pure spec/compile/validate model (no DOM, no imports) — Comfy Bridge gets a GENERATED copy via `node scripts/sync-theme-spec.js`; never edit the bridge copy. `theme-studio.ts` is the modal; its compiled var map is filtered to `THEME_VARS` + `themes.ts` `CUSTOM_GRAMMAR_KEYS` (the same list `clearCustomOverrides()` clears — a new compiled key must be added there). `--c-*` vars are read only by the `html[data-theme="custom"]` block above `End themes`. Fills/card hovers have a `tier` and cost Edibits once to save (`dts-custom-fx-owned`, `achievements.ts` `spendEdibits`). Saved Customs live in `dts-custom-library`; the active one is mirrored into the `dts-custom-theme*` keys by `persistActive()` and other ones appear in `#themeSelect` as `custom:<id>` options. Full detail: notes/Systems/Themes-System.md.

**Past Tag Preview (ghost chips):** `canonical-tags.ts` `ghostTagsFor(entry)` mirrors `unmergeChildren()`'s evidence exactly — keep the two in sync (both honor `entry.meta.ghostDismissed`). Ghost deletes log `'ghost-remove'`, undone through the REVIEW_TYPES meta applier. **Android app = generated, not forked (2026-09-27):** `mobile/sync-web.js` builds `mobile/www/` from `renderer/` every run (`<html class="mobile-app">` + shim). Mobile-only CSS goes in `renderer/styles.css`'s "Phones & the Android app" block (`html.mobile-app` / `html.touch-device` / width media); never edit `mobile/www/`. Every `on*` hook in `src/preload.ts` needs a no-op in `mobile/mobile-shim.js` (unguarded boot subscriptions).
...
`comfy-client.ts`, and the mobile shell's own XHR transport via `BridgeShared.*` — XHR for upload
progress; it MUST honor `init.timeoutMs` (as a stall timeout) and take an abort signal, since a
fetch transport that ignored `timeoutMs` let a stalled reference upload wedge a run forever —
see `notes/Apps/Comfy-Bridge.md`). Only the
...
**Aspect-ratio bucketing ("Bucket Images") — the renderer owns the dataset's file I/O; the main process only computes.** The dataset folder is a `FileSystemDirectoryHandle` held by the renderer, so `src/bucket-local.ts` (main) has NO path to it — `bucketImage()` takes image BYTES and returns PNG BYTES, and `src/renderer/bucket-images.ts` does every move/copy/write. Exactly the WD14 split; don't try to pass a folder path across IPC. `src/bucket-core.ts` holds the pure bucket math (`getValidBuckets`/`getBestBucket`/`isBucketSize`), imported by both sides. **GPU by default, CPU fallback** — the dock's **Prefer GPU** checkbox (persisted `localStorage['dts-bucket-settings']`, default on) builds u2net's session via DirectML and falls back to CPU inside the same call on a throw *or* a 45s timeout; the winning provider rides back on `BucketImageResult.provider`. DirectML's session build for u2net used to hang silently (burning CPU, freezing main, no error) — it now builds in ~4s / infers in <1s on the verified Windows box under `onnxruntime-node` 1.30.0, but the 45s timeout guard must STAY, since a re-hang is invisible to try/catch (`notes/Pitfalls/DirectML-Hangs-On-U2Net-Graph.md`). Bucketing originals land in `original_images/` as entries that are `disabled: true` AND `original: true` — disabled so every existing mass/auto tool skips them, `original` so they get their own view instead of the Disabled one. Any new `disabled`-keyed loop that should also ignore originals must check `.original` too. **A bucketed image and its original share the same filename stem**, so `base` is not unique once `original_images/` exists — `buildEntry()` scans originals last and must NOT let an `original` entry clobber `entryByBase[base]` (guard: `if (!(original && entryByBase.has(base))) entryByBase.set(base, entry)`), or every `getEntryByBase(base)` selection-based action (Apply/remove on selected, Lock, Disable, Delete) silently targets the hidden disabled original while the toast + edit log still report success. Full note: `notes/Features/Bucket-Images.md`; see `notes/Pitfalls/EntryByBase-Collides-With-Original-Images.md`.

**Flagged For Review list (`src/renderer/tag-index.ts`, toggled by `#btnReviewFlagged` next to `#btnFlagIsolated`).** While active it HIJACKS the left-panel TAGS list — `renderTagFrequencyList` early-returns into `renderFlaggedReviewList`, showing the union of every entry's `meta.flaggedTags` (alphabetical, heading "FLAGGED FOR REVIEW", family/count sort controls hidden via `#tagFamilyListArea.review-mode`). Rows are NOT clickable; each carries a **Reviewed** button that strips the tag from every entry's `meta.flaggedTags` and records ONE undoable `'unflag-review'` edit-log entry. Key rule: that log type swaps `meta.flaggedTags`, NOT `entry.tags`, so it MUST use its own `REVIEW_TYPES` set + `applyFlaggedReviewDirection` applier (injected from `index.ts`, the same shape as PIXEL_TYPES/ISOLATE_TYPES) — never ride `TAG_TYPES`/`applyTagDirection`, which would overwrite the caption with the flagged list. It's in `STAT_CHART_COLORS`/`STAT_TYPE_LABEL` but deliberately NOT `TAG_TYPES`. `reviewedFlaggedTags` is session-only (a cleared row stays struck-through), cleared by `resetReviewFlagged()` on folder load/unload and on each switch into the mode. Full note: `notes/Features/Flagged-For-Review.md`.

**Tag Sorting (Single mode + card modal).** A 🏷 toggle (`view.ts`'s `tagSortingActive`, persisted `localStorage['dts-tag-sorting']`) switches the chips block (`buildChipsBlock`, shared by `renderSingleView` and `renderImageCardModal` — grid cards stay flat) from one flat chiprow to one labelled segment per prompt-field category via `tag-categories.ts`'s `groupTagsByCategory()`. Classification is 3-stage: exact seed map `TAG_CATEGORY_SEEDS` (9.1k tags from a Danbooru tag-group snapshot) → word-boundary keyword rules `TAG_CATEGORY_RULES` → `'other'`. BOTH live in **`src/renderer/tag-categories-data.ts`, a GENERATED file — never hand-edit it**; its inputs are `scripts/tag-categories/raw/<cat>.json` (committed Danbooru snapshot) + `scripts/tag-categories/rules/<cat>.json`, compiled by `node scripts/build-tag-categories.js` (`npm run build:tag-categories`). Precedence is **sexual > limbs > pose > clothes > face > body > character > scene > effects > other** — LIMBS deliberately outranks pose because Danbooru's Posture page cross-lists gesture tags (else "waving"/"salute" file under Pose), and FACE outranks body so the face/eyes/ears/eyebrows/nose groups land in the Face header (hair stays Body). Danbooru is bot-protected: direct Node/PowerShell HTTP is 403, only a browser-like fetcher gets through, hence the committed snapshot. Adding a category = raw+rules files + build-script `ORDER` + `TAG_CATEGORY_LABELS` + `TAG_CATEGORY_ORDER`. **Multi-subject headers are BUILT (opt-in):** `buildChipsBlock` branches to `view.ts`'s `buildSubjectTree` when `entry.meta.tagSubjects` is non-empty. User-named subject headers (inline-renamable) each hold category subheaders (added via `openSubheaderPicker`); tags default to the FIRST subject by classifier category, and are moved manually by dragging a chip onto a subject block or shift-clicking chips + "Move tags to:" flyout. Persisted per image in `entry.meta` (`tagSubjects` + `tagAssign`, see `types.ts`'s `TagSubject`) via `saveEntryMeta`. Removing every subject returns to the flat list. Full note: `notes/Features/Tag-Sorting.md`.

**Single mode: static preview + click-to-lightbox (2026-09-24).** Normal Single view no longer has an inline zoomable image. `view.ts`'s `buildSinglePreview(e)` renders a small static preview (`.single-preview`); clicking it opens `shared-ui.ts`'s `showImageLightbox(e.objectUrl)` (fullscreen, wheel/pinch zoom, drag-pan, Esc/click-outside to close) — the same lightbox sequential mode and the card modal already use. The freed space widens the tag panel (`.single-panel-main` = flex:1) so it scrolls far less. The old zoom slider/`applySingleTransform` were removed, and `buildSingleImgSide` is GONE — sequential mode uses the same `buildSinglePreview` (2026-09-24). The "Zoom Zoom" achievement's `folderStats.zoom_max` is now fed by the lightbox's `onZoom` callback → `recordZoom()`. Re-rendering the SAME image (Single view or the card modal) keeps the panel's scroll via `capturePanelScroll(host)`. The Tag Sorting bar (`buildTagSortBar`) holds the toggle + "＋ Add subject".

**Theme-wide CSS rules (2026-09-24).** The epic/legendary/refined `button:not(...)` rule uses `overflow: clip`, NOT `hidden` — `hidden` makes a scroll container, zeroing a flex item's automatic min size and squashing buttons below their own text. Every checkbox gets the themed look from a zero-specificity `:where(input[type="checkbox"])` rule (class sizes still win); standalone checkbox+text toggles are wrapped in `label.ach-toggle-row`. The toolbar counter `#singlePos` is built by `renderSinglePos()` as an editable field: type a number + Enter/blur to jump (`singleIndex` clamp + re-render).
