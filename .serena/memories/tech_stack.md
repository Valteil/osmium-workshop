...
- `src/renderer/index.ts` and every extracted renderer module carry `// @ts-nocheck` — the
  TypeScript port so far is mechanical (real `.ts` files, real ES modules) but NOT type-annotated;
  don't assume type-checking catches renderer bugs.
